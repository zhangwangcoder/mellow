...
function startCore(callback) {
    ...
    if (isWin32) {
        ...
        try {
            out = await
            sudoExec(util.format('"%s" "%s" %s', path.join(helperResourcePath, 'ensure_tap_device.bat'), path.join(helperResourcePath, 'tap-windows6'), tunName))
            log.info(out)
        } catch (err) {
...
function configRoute() {
    ...
    try {
        switch (process.platform) {
            ...
            case 'win32':
                await
                sudoExec(util.format('"%s" %s %s', path.join(helperResourcePath, 'config_route.bat'), tunGw, tunName))
                break
            case 'linux':
    ...
    if (origGw !== null) {
        ...
        try {
            switch (process.platform) {
                ...
                case 'win32':
                    await
                    sudoExec(util.format('"%s" %s', path.join(helperResourcePath, 'recover_route.bat'), tunName))
                    break
                case 'linux':
...
:
    ...
    >
        {
            store.set('autoConnect', item.checked)
        }
    ,
...
,
    ...
    ,
        ...
        >
            {
                store.set('sniffing', item.checked)
            }
        ,
    ...
    ,
        ...
        >
            {
                store.set('fakeDns', item.checked)
            }
        ,
...
,
    {
        label: i18n.t('Check For Updates'), type
    :
        'normal', click
    :
